...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if n > max_length:
            sen = sentence[:max_length]
            mask = [True] * max_length
            labels = labels[:max_length]
        elif n < max_length:
            sen = sentence + [zero_vector] * (max_length - n)
            mask = [True] * n + [False] * (max_length - n)
            labels = labels + [zero_label] * (max_length - n)
        else:
            sen = sentence
            mask = [True] * n
            labels = labels

        ret.append((sen, labels, mask))
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        embedding_table = tf.Variable(self.pretrained_embeddings)
        embeddings = tf.nn.embedding_lookup(params=embedding_table, ids=self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        ### COULD BE WRONG
        #h = tf.constant(0.0, shape=[tf.shape(x)[0], self.config.hidden_size])
        #h = tf.zeros_like(h)
        h = tf.zeros([tf.shape(x)[0], self.config.hidden_size])
        #h = tf.get_variable('h', shape=[tf.shape(x)[0], self.config.hidden_size],
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                ot, h =  cell(xt, h, scope='RNN')        
                drop_ot = tf.nn.dropout(ot, 1-dropout_rate)       
                yt = tf.matmul(drop_ot, U) + b2
