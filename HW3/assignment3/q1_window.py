...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for i in range(window_size, n + window_size):
            data_point = sum(pad_sentence[i-window_size: i+window_size+1], [])
 
            windowed_data.append((data_point[:], labels[i-window_size]))             
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        embeddings_table = tf.Variable(self.pretrained_embeddings, name='embeddings_table')
        embeddings = tf.nn.embedding_lookup(params=embeddings_table, ids=self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        b1 = tf.get_variable("b1", shape=(self.config.hidden_size), initializer = tf.contrib.layers.xavier_initializer(seed=2))
        ...
        b2 = tf.get_variable("b2", shape=(self.config.n_classes), initializer = tf.contrib.layers.xavier_initializer(seed=4))
