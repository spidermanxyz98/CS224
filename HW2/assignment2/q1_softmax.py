...
def test_cross_entropy_loss_basic():
    ...
    class_label = np.random.choice(n_class, n_samples)
    ...
    yhat_init = np.random.rand(n_samples, n_class)
    #calculate softmax
    yhat = np.exp(yhat_init) / np.sum(np.exp(yhat_init), axis = 1, keepdims=True) 
    test2 = cross_entropy_loss(tf.constant(y, dtype=tf.int32), tf.constant(yhat, dtype=tf.float32))
    
    expected2 = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits_v2(labels=tf.constant(y, dtype=tf.int32), logits=tf.constant(yhat_init, dtype=tf.float32)))
    with tf.Session() as sess:
        test2_eval= sess.run(test2)
        expected2_eval = sess.run(expected2)
    test_all_close("Cross-entropy test 2", test2_eval, expected2_eval)
